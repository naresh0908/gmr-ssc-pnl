...
export default function EBITMatrix() {
  const { derived, year } = useDashStore()
  const Y = derived.byYear[year]
  if (!Y) return null

  const matrix = Y.ebitMatrix
  ...
  return (
    <div className="mt-7">
      <SectionHead num="01" title={`EBIT Matrix · Month × Department · FY ${year}`}>
        Per-month EBIT contribution (₹ Cr) by service line. Hover any cell to see contribution share.
      </SectionHead>
